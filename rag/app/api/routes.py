...
# ── Async index task store ───────────────────────────────────────
_INDEX_TASKS: dict[str, dict] = {}


@router.post("/index-all")
async def index_all(req: dict = {}):
    """Start async full/incremental index. Returns task_id immediately.
    body: {"full": true|false}.
    """
    from app.indexer.doc_indexer import index_directory
    from app.retrievers.es_retriever import get_es_client, INDEX_NAME, init_index
    ...
    async def _run():
        try:
            ...
            if full:
                ...
                init_index(es)
                _INDEX_TASKS[task_id]["progress"] = "full_rebuild"
            _INDEX_TASKS[task_id]["progress"] = "indexing"
            ok, err = await asyncio.wait_for(
                index_directory(es, "/app/wiki", clean=False),
                timeout=settings.index_timeout_seconds,
            )
            _INDEX_TASKS[task_id].update(status="completed", success=ok, failed=err,
                                         finished_at=datetime.now(timezone.utc).isoformat())
        except asyncio.TimeoutError:
            _INDEX_TASKS[task_id].update(status="failed", error="timeout",
                                         finished_at=datetime.now(timezone.utc).isoformat())
        except Exception as e:
            _INDEX_TASKS[task_id].update(status="failed", error=str(e),
                                         finished_at=datetime.now(timezone.utc).isoformat())
...
async def topology_all():
    """Return all services, edges, hosts for global topology view."""
    driver = get_driver()
    try:
        result = get_all_service_topology(driver)
        if "error" in result:
...
async def hosts_status():
    """Return all hosts with their availability status.
    Currently reads from Neo4j. Future: merge with Zabbix available flag."""
    driver = get_driver()
    try:
        result = get_all_service_topology(driver)
...
async def alerts_active():
    ...
    Future: merge with Zabbix triggers."""
    return {
        "alerts": [],
        "total": 0
    }
...
async def send_message(conv_id: str, data: dict):
    ...
    Supports multi-turn via context in body."""
    ...
    # Call query endpoint to get answer
    from app.api.routes import query as _query
    from app.models.query import QueryRequest
    req = QueryRequest(query=query, top_k=data.get("top_k", 5))
    qresp = await _query(req)

    # Save assistant message
    import json
    _add_msg(conv_id, "assistant", qresp.answer, [
        {"title": s.title, "score": s.score, "engine": s.engine}
        for s in (qresp.sources or [])
    ])

    # Token estimation (rough: char count / 2)
    total_chars = len(query) + len(qresp.answer)
    _upd_tokens(conv_id, total_chars // 2)

    # Re-read updated conversation
