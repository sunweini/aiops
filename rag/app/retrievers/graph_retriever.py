...
def get_all_service_topology(driver) -> dict:
    """Return all services + edges for global topology view."""
    try:
        with driver.session() as session:
            services = session.run(
                "MATCH (s:Service) RETURN s.id AS id, s.name AS name, s.description AS description"
            ).data()
            edges = session.run(
                """
                MATCH (s:Service)-[c:CALLS]->(t:Service)
                RETURN s.id AS source, t.id AS target,
                       c.protocol AS protocol, c.port AS port
                """
            ).data()
            hosts = session.run(
                "MATCH (h:Host) RETURN h.id AS id, h.name AS name, h.ip AS ip"
            ).data()
            deploys = session.run(
                """
                MATCH (s:Service)-[:DEPLOYS_ON]->(h:Host)
                RETURN s.id AS service_id, h.id AS host_id
                """
            ).data()
            return {
                "services": services,
        ...
        print(f"Neo4j all-topology error: {e}")
